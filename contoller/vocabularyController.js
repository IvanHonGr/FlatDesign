...
mongoose.connect('mongodb://test:test@ds161939.mlab.com:61939/todo');
...
module.exports = function (app) {
    ...
    app.post('/vocabulary/manage', urlEncoderParser, function (req, res) {
        Words.findOneAndUpdate({ word: req.body.word }, req.body, { upsert: true }, function (err, doc) {
            if (err) throw err;
            res.json();
        });
    });
    ...
    app.post('/vocabulary/manage/del', urlEncoderParser, function (req, res) {
        Words.remove({ word: req.body.word }, function (err) {
            if (err) throw err;
            res.json();
        });
    });
    ...
    app.post('/vocabulary/training/pick', urlEncoderParser, function (req, res) {
        ...
        Words.find({ word: doc.word }, function (err, data){
            if (doc.correctTimes) {
            ...
            Words.findOneAndUpdate({ word: doc.word }, doc, { upsert: true }, function (err, doc) {
                if (err) throw err;
                res.json();
            });
        });
